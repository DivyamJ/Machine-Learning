...
n = 3   # n = number of layers

X = []
xx = np.ndarray(shape=(len(V)+1,1))
X.append(xx)
hl_1 = int(input('How many neurons do you want in the first hidden layer?'))
xx = np.ndarray(shape=(hl_1+1,1))
X.append(xx)
hl_2 = int(input('How many neurons do you want in the second hidden layer?'))
xx = np.ndarray(shape=(hl_2+1,1))
X.append(xx)      
xx = np.ndarray(shape=(2,1))
X.append(xx)
...
def forward(zero_layer):
    ...
    for i in range(1,n):
        for j in range(1,len(X[i])):
            X[i][j][0]=1/(1+math.exp(-1*np.matmul(X[i-1].T,W[i-1][:,j-1])[0]))
    
    for j in range(0,len(X[n])):
        #print('cccccccccccc')
        #print(np.matmul(X[n-1].T,W[n-1][:,j])[0])
        X[n][j][0]=math.exp(np.matmul(X[n-1].T,W[n-1][:,j])[0])
    
    total = np.sum(X[n])
    
    for j in range(0,len(X[n])):
        X[n][j][0]=X[n][j][0]/total
...
def backward(actual):           
    ...
    #print('Holoooooo')
    #print(X[n][0][0])
    if (X[n][0][0]==1 and actual==1) or (X[n][1][0]==1 and actual==0):
        error = 0 
    else:
        error = -(actual*math.log(X[n][0][0])+(1-actual)*math.log(X[n][1][0]))

    delta[n-1]=np.ndarray(shape=(2,1))

    delta[n-1][0][0]=(X[n][0][0]-actual)
    delta[n-1][1][0]=(X[n][1][0]-(1-actual))
    ...
    for i in range(n-1,0,-1):
        ...
        for j in range(1,len(X[i])):
            ...
            delta[i-1][j-1][0]=X[i][j][0]*(1-X[i][j][0])*delta[i-1][j-1][0]
...
def accuracy(index):
    ...
    for i in range(len(X_train)):
            forward(rep_in[i])
            if X[n][0][0]>X[n][1][0]:
                predicted_train[index].append(1)
            else:
                predicted_train[index].append(0)
    ...
    for i in range(len(X_test)):
            ...
            if X[n][0][0]>X[n][1][0]:
                predicted_test[index].append(1)
            else:
                predicted_test[index].append(0)
