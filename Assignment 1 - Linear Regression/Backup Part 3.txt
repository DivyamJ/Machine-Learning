...
for k in [10,100,1000,10000]:
  ...
  # Splitting the dataset into training set and test set
    ...
    #Finding the matrix of parameters
    
    '''Degree of polynomial chosen is 9 since it turned out to be the best as per our previous analysis'''
    ...
    phi_x = np.array([[elem**i for i in range(0,10)] for elem in X])
    phi_x = phi_x.T
    y_pred = np.matmul(W,phi_x)
    
    phi_xtrain = np.array([[elem**i for i in range(0,10)] for elem in X_train])
    ...
    plt.title('Visualising the curve: degree = 9, number of instances = %i' %k)
    plt.xlabel('x values')
    plt.ylabel('y values')
    plt.show()
   
plt.plot([10,100,1000,10000], Train_error, color = 'red')
plt.plot([10,100,1000,10000], Test_error, color = 'blue')
plt.title('Training error(red) and Test Error(blue) for number of instances ')
